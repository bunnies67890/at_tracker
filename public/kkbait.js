...
// Search Handler with Blue Route Header Button
...
function handleSearch() {
  ...
  if (query.length > 0) {
    ...
    html += `<div class="search-route-header" onclick="openRouteHistoryModal('${routeQuery}')">View Full Route History for Route ${routeQuery}</div>`;
  }
...
async function openRouteHistoryModal(routeDisplay, specificTrip = null) {
  ...
  if (modalTitle) modalTitle.innerText = `Route ${routeDisplay} History`;
...
async function loadRouteHistoryForSelectedDate() {
  ...
  try {
    ...
    if (!shifts || shifts.length === 0) {
      departuresList.innerHTML = `<p style="color: #666; text-align: center; padding: 20px;">No recorded history for Route ${activeRouteDisplay} on ${selectedDate}.</p>`;
      return;
    }
      ...
      detailsBox.innerHTML = `
        ...
        <ul style="margin: 0; padding-left: 18px; display: flex; flex-direction: column; gap: 5px;">
          ${history.slice(0, 5).map(h => {
            ...
            // Clean destination text using the same helper function
            ...
            return `
              <li style="${isThisRun ? 'color: #d9381e;' : ''}">
                @ ${h.start_time} - Route ${h.route_display}${destDisplay} (${h.tardiness})
